...
class NHentaiSearchMetadata : RaisedSearchMetadata() {
    ...
    override fun getExtraInfoPairs(strings: MetadataStringProvider): List<Pair<String, String>> {
        return with(strings) {
            listOfNotNull(
                getItem(nhId) { string(SYMR.strings.id) },
                getItem(
                    uploadDate,
                    {
                ...
                getItem(favoritesCount) { string(SYMR.strings.total_favorites) },
                getItem(mediaId) { string(SYMR.strings.media_id) },
                getItem(japaneseTitle) { string(SYMR.strings.japanese_title) },
                getItem(englishTitle) { string(SYMR.strings.english_title) },
                getItem(shortTitle) { string(SYMR.strings.short_title) },
                getItem(coverImageType) { string(SYMR.strings.cover_image_file_type) },
                getItem(pageImageTypes.size) { string(SYMR.strings.page_count) },
                getItem(thumbnailImageType) { string(SYMR.strings.thumbnail_image_file_type) },
                getItem(scanlator) { string(MR.strings.scanlator) },
            )
        }
    }
