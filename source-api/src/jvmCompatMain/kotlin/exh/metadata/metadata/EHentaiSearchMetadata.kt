...
import exh.pref.japaneseTitlesEnabled
...
class EHentaiSearchMetadata : RaisedSearchMetadata() {
    ...
    override fun getExtraInfoPairs(strings: MetadataStringProvider): List<Pair<String, String>> {
        return with(strings) {
            listOfNotNull(
                getItem(gId) { string(SYMR.strings.id) },
                getItem(gToken) { string(SYMR.strings.token) },
                getItem(exh) { string(SYMR.strings.is_exhentai_gallery) },
                ...
                getItem(genre) { string(SYMR.strings.genre) },
                getItem(
                    datePosted,
                    {
                ...
                getItem(parent) { string(SYMR.strings.parent) },
                getItem(visible) { string(SYMR.strings.visible) },
                getItem(language) { string(SYMR.strings.language) },
                getItem(translated) { string(SYMR.strings.translated) },
                getItem(size, { MetadataUtil.humanReadableByteCount(it, true) }) {
                    string(SYMR.strings.gallery_size)
                },
                ...
                getItem(aged) { string(SYMR.strings.aged) },
                getItem(
                    lastUpdateCheck,
                    {
                        MetadataUtil.EX_DATE_FORMAT
                ...
                ) { string(SYMR.strings.last_update_check) },
            )
        }
    }
    ...
    companion object {
        ...
        private fun splitGalleryUrl(url: String) =
            url.let {
                ...
                val path = if (it.startsWith("http")) {
                    it.substringAfter("://")
                        .substringAfter("/")
                        .substringBefore("?")
                        .substringBefore("#")
                ...
                path.split('/').filterNot(String::isNullOrBlank)
            }
