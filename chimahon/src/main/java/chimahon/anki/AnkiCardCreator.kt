...
object AnkiCardCreator {
    ...
    private fun buildSentenceFuriganaHtml(sentence: String, session: Long): String {
        ...
        val results = HoshiDicts.lookup(session, sentence, 99, 25).toList()
        val matches = greedySentenceMatches(sentence, results)
        val sb = StringBuilder()
        var i = 0
        while (i < sentence.length) {
            val m = matches[i]
            if (m != null && i + m.matched.length <= sentence.length) {
                if (m.matched == m.term.expression) {
                    sb.append(buildFuriganaHtml(m.term.expression, m.term.reading))
                } else {
                    sb.append(escapeHtml(m.matched))
                }
                i += m.matched.length
            } else {
    ...
    private fun buildSentenceFuriganaPlain(sentence: String, session: Long): String {
        ...
        while (i < sentence.length) {
            ...
            if (m != null && i + m.matched.length <= sentence.length) {
                if (m.matched == m.term.expression) {
                    sb.append(buildFuriganaPlain(m.term.expression, m.term.reading))
                } else {
                    sb.append(m.matched)
                }
                i += m.matched.length
    ...
    private fun greedySentenceMatches(
        sentence: String,
        results: List<LookupResult>,
    ): Map<Int, LookupResult> {
        val raw = mutableMapOf<Int, LookupResult>()
        for (r in results) {
            val pos = sentence.indexOf(r.matched)
            if (pos < 0) continue
            val cur = raw[pos]
            if (cur == null || r.matched.length > cur.matched.length) raw[pos] = r
        }
        return raw
    }
