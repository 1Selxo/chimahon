...
data class ChimahonSubtitleRegexFilterOptions(
...
fun ChimahonPlayerSubtitleSettings.subtitleRegexFilterOptions(): ChimahonSubtitleRegexFilterOptions {
    return ChimahonSubtitleRegexFilterOptions(
        removeSpeakerNames = regexRemoveSpeakerNames,
        mergeMultiline = regexMergeMultiline,
        removeBracketedText = regexRemoveBracketedText,
        removeUppercaseLines = regexRemoveUppercaseLines,
        removeMusicSymbols = regexRemoveMusicSymbols,
        removeCurlyBracedText = regexRemoveCurlyBracedText,
        customRegexEnabled = regexCustomEnabled,
        customRegexPattern = regexCustomPattern,
    )
}

fun String.applyChimahonSubtitleRegexFilters(settings: ChimahonPlayerSubtitleSettings): String {
    return applyChimahonSubtitleRegexFilters(settings.subtitleRegexFilterOptions())
}

fun String.applyChimahonSubtitleRegexFilters(options: ChimahonSubtitleRegexFilterOptions): String {
    ...
    if (options.customRegexEnabled && options.customRegexPattern.isNotBlank()) {
        customChimahonSubtitleRegex(options.customRegexPattern)?.let { regex ->
...
fun customChimahonSubtitleRegex(pattern: String): Regex? {
    return runCatching {
